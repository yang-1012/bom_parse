...
5. **导出结果** — 点击"导出结果"，生成 Excel 报表，包含"分类明细"和"统计汇总"两个 Sheet
...
| [config/coefficients.json](config/coefficients.json) | 折算系数：封装类型 → 折算系数的映射 |
| [config/pin_count_rules.json](config/pin_count_rules.json) | 管脚数映射：封装类型 → 管脚数的映射 |
